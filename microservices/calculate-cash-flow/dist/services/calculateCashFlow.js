...
exports.calculateCashFlow = void 0;
const holidays = [
    "2023-01-01",
    "2023-01-16",
    "2023-02-20",
...
function dateMod(date, typeOfDay) {
    let newDate = date;
    let dayInc = 0;
    if (typeOfDay == "Non-holiday weekday before")
        dayInc = -1;
    else if (typeOfDay == "Non-holiday weekday after")
        dayInc = 1;
    if (dayInc != 0) {
        while (true) {
...
const calculateCashFlow = (expenseRules) => {
    ...
    */
    const relevantStartDate = new Date();
    relevantStartDate.setUTCDate(today.getUTCDate() - 4);
    const relevantEndDate = new Date();
    relevantEndDate.setUTCDate(today.getUTCDate() + 184);
    for (let rule of expenseRules) {
        // Set startDate & endDate if they are empty
        if (!rule.startDate)
            rule.startDate = customDateString(relevantStartDate);
        if (!rule.endDate)
            rule.endDate = customDateString(relevantEndDate);
        // Skip rule if not within relevant dates
        ...
        // Set "every" to default value if it was empty
        if (!rule.every)
            rule.every = 1;
        // Set "typeOfDay" to default value if it was empty
        if (!rule.typeOfDay)
            rule.typeOfDay = "";
        // Set start date based on "onDate" if there is one
        let expenseDate = new Date();
        if (rule.freq == "Yearly" && rule.onDate && rule.onDate > 99) {
            const year = relevantStartDate.getFullYear();
            const ruleOnMonth = Math.floor(rule.onDate / 100) - 1;
            // Typescript uses months 0-11, while database is 1-12
            const ruleOnDay = rule.onDate % 100;
            expenseDate = new Date(Date.UTC(year, ruleOnMonth, ruleOnDay, 0, 0, 0));
            // Note, days past end of month will automatically roll to proper date next month
            ...
            const year = new Date(`${rule.startDate}T00:00`).getUTCFullYear();
            const month = new Date(`${rule.startDate}T00:00`).getUTCMonth();
            // 0th day of next month is last day of this month
            const lastDayOfMonth = new Date(Date.UTC(year, month + 1, 0, 0, 0, 0)).getUTCDate();
            let effectiveDay = lastDayOfMonth;
            if (effectiveDay > rule.onDate)
                effectiveDay = rule.onDate;
        ...
        // Create expenses
        while (expenseDate <= relevantEndDate &&
            expenseDate <= new Date(`${rule.endDate}T00:00`)) {
            if (expenseDate >= new Date(`${rule.startDate}T00:00`) &&
